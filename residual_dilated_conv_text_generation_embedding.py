...
from keras.layers import Flatten, Dense, Input, merge, Activation, Embedding, Conv1D
...
def res_dilated_conv_model(max_features, max_len, embedding_dims=20):
    input = Input(shape=(max_len,), dtype='int32', name='main_input')
    seq = Embedding(max_features,
                    embedding_dims,input_length=max_len)(input)
    res_layers = []
    for i in range(8):
    ...
    seq = Flatten()(seq)
    seq = Dense(max_features, activation='softmax')(seq)
...
if __name__ == '__main__':
    ...
    model = res_dilated_conv_model(len(chars), maxlen)
