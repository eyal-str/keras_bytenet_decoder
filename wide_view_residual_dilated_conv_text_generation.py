...
from keras.layers import Flatten, Dense, Input, merge, Activation, Conv1D, Lambda, concatenate, Permute
...
def res_dilated_conv_model(input_shape, drill_down_last_n_chars):
    input = Input(input_shape)

    cut_sample = K.reshape(np.eye(input_shape[0], input_shape[0], dtype='float32')
                           [:, input_shape[0] - drill_down_last_n_chars:],
                           (input_shape[0], drill_down_last_n_chars))

    input_ = Permute((2, 1))(input)
    partial_input = Lambda(lambda x: (K.dot(x, cut_sample)), name='split_layer',
                           output_shape=(input_shape[1], drill_down_last_n_chars))(input_)
    seq = Permute((2, 1))(partial_input)
    full_embedd_input = Dense(256)(input)
    res_layers = []
    for i in range(8):
        seq, curr_layer = res_dilated_block_1(256, 2**i, 5 if i == 0 else 2)(seq)
        res_layers.append(curr_layer)

    seq = concatenate([full_embedd_input, seq],axis=1)
    seq = Flatten()(seq)
    seq = Dense(input_shape[1], activation='softmax', name ='yoyo2')(seq)
...
if __name__ == '__main__':
    ...
    model = res_dilated_conv_model((maxlen, len(chars)), drill_down_last_n_chars = 125)
